...
defineProps(['SkillValuesForm','showSkillAddedd','errorMessage',
'AddeddSkillCard','skillErrorMessage','SkillEdit','skills','dialogSkill']);
defineEmits(['closeSkillsDialog','addSkill','addNewSkill','clearSkills','removeSkill','removeSkillData']);
...
<form>   
  ...
  <div class="relative">
      ...
      <input v-model="SkillValuesForm.Skill" type="text" id="default-skill" class="block w-full p-4 pl-10 text-sm text-gray-900 border
       border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 
       dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Add your skills here ..." required>
      <button
      v-if="SkillEdit"
      type="button" 
      @click="$emit('addNewSkill',SkillValuesForm, $page.props.auth.resumeID)"
...
<div v-show="SkillEdit" class="max-w-full p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
  <div class="">
      <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Your skills: </h5>
    
        <span  v-for="(skillItem, index) in skills"
